        ...
        <Modal center open={isOpen} onClose={onClose}>
          {
            ...
            <h2 className="font-semibold">Información Detallada del Producto</h2>
            <p className="italic text-sm text-start mb-4 w-[400px]">
            Revisa la información completa del producto seleccionado. Esta sección incluye el nombre, cantidad, categoría, observaciones adicionales y el usuario responsable de su registro.</p>
          </div>
          ...
          <CustomTextfieldComponent
            ...
            onChange={(_) => _}
            value={
              inventarioResponse?.observacion_general ? inventarioResponse?.observacion_general : 'No se asignó ninguna Observacion'
            }
            />
             <CustomTextfieldComponent
            disabled
          ...
          {
            inventarioResponse?.imgUrl ? <DropzoneImagePreview filePreview={inventarioResponse.imgUrl} name={inventarioResponse.nombre_producto}/> 
            
            : (<p className='font-semibold text-sm text-gray-600 pb-1 block'>No se registró ninguna Imagen.</p>)
          }
          </>  
          }
