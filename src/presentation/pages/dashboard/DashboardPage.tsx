...
const DashboardPage = () => {
  ...
  const getBarCard = async (): Promise<BarGraphResponse> => {
    const resp = await Api.instance.get<BarGraphResponse>('/api/inventario/metrics/bar');
    const data = await resp.data;
  ...
  const data = {
    ...
    datasets: [{
      ...
      data: [totalProducts?.totalProductsOnAirbnb, totalProducts?.totalProductsOnCafe, totalProducts?.totalProductsOnCocina, totalProducts?.totalProductsOnInmobiliaria, totalProducts?.totalProductsOnLimpieza, totalProducts?.totalProductsOnPlateria,totalProducts?.totalProductsOnRestaurante, totalProducts?.totalProductsOnUtensillos ],
  ...
  <div className='home-container flex flex-wrap justify-evenly items-start  mt-2 '>
   ...
   <h1 className='font-semibold text-lg text-start'>Métricas Inventario</h1>
   <p className='font-normal italic text-xs mt-2 mb-2'>Análisis detallado de la distribución del inventario por categoría, presentado a través de un gráfico que muestra en porcentaje las áreas con mayor asignación de recursos.</p>
   </div>
   ...
   <h1 className='font-semibold text-lg'>Métricas Inventario</h1>
      <p className='font-normal italic text-xs mt-2 mb-2'>En este gráfico, se destacan las fechas con mayor inversión, lo que permite identificar patrones y tendencias clave en la gestión de inventario.</p>
      </div>
     <Bar  data={dataBar}/>
   </DashboardCard>
  </div>
